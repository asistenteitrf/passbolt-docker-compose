...
class PassboltAPI:
    ...
    def __init__(self, fingerprint):
        ...
        self.resources_url = f'{self.base_url}/resources.json'
    ...
    def get_resources(self):
        response = self.session.get(self.resources_url)
        decoded_response = json.loads(response.text)
        return decoded_response['body']
    
    def PassWord(self,id): 
        result = str(self.decrypt(id))           
        return result[result.find('"password":')+12:result.find('}')-1].replace(" ","").replace("\\t","")
    
    def Get_Resource_Info(self,search_resource):
        resources = self.get_resources()
         
        for resource in resources:
            if (search_resource==resource['name']):
                token={"user_resource": 'TEMU\\' + resource['username'],
                       "pass_resource": self.PassWord(self.get_resource_secret(resource['id'] )) } 
                return token
        return False
